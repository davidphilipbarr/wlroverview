...
def normalize_title(s: str) -> str:
    """
    Normalize a window title string.

    Performs NFKC normalization, removes zero-width characters,
    unifies dash characters, and collapses whitespace.
    """
    if not s:
        return ""
...
def load_user_css():
    """
    Load user-defined CSS from ~/.config/wloverview/style.css if it exists.
    """
...
class MainWindow(Gtk.Window):
    """
    Main application window enabling overview of open windows.
    """
    def __init__(self):
        super().__init__()
...
# Bluetooth
    ...
    def add_sys_button(self, parent, icon_name, callback):
        """
        Helper to create and add a uniform system button to the UI.
        """
    ...
    def _wlrctl_focus_try(self, appid: str, title: str | None):
        ...
        if appid and title:
            for k in key_variants:
                tries.append(["wlrctl", "toplevel", "focus", f"{k}:{appid}", f"title:{title}"])
        if title:
    ...
    def focus_appid_best(self, appid: str):
        """
        Dock focusing: pick an ACTUAL running window for this appid and focus it using appid+title.
        This avoids accidentally focusing wloverview when appid match is empty/wrong/broad.
        """
        wins = get_windows()
    ...
    def build_dock(self, overlay):
        ...
        for e in cfg:
            ...
            def on_click(g, _n_press, _x, _y, cmd=cmd):
